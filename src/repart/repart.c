...
static Partition* partition_free(Partition *p) {
        ...
        copy_files_line_free_many(p->copy_files, p->n_copy_files);
...
static bool partition_needs_populate(const Partition *p) {
        ...
        return p->n_copy_files != 0 || !strv_isempty(p->make_directories) || !strv_isempty(p->make_symlinks) ||
...
static int do_copy_files(Context *context, Partition *p, const char *root) {
        _cleanup_strv_free_ char **subvolumes = NULL;
        _cleanup_free_ CopyFilesLine *override_copy_files = NULL;
        const CopyFilesLine *copy_files = p->copy_files;
        int n_copy_files = p->n_copy_files;
        int r;
        ...
        if (p->suppressing) {
                ...
                assert(ptr == &override_copy_files[n_copy_files]);

                copy_files = override_copy_files;
        }
...
static int context_mkfs(Context *context) {
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                r = make_filesystem(partition_target_path(t), p->format, strempty(p->new_label), root,
                                    p->fs_uuid, MKFS_DISCARD * arg_discard | MKFS_FS_VERITY * p->need_fsverity,
...
static int context_minimize(Context *context) {
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                r = make_filesystem(d ? d->node : temp,
                                    ...
                                    root,
                                    fs_uuid,
                                    MKFS_DISCARD * arg_discard | MKFS_FS_VERITY * p->need_fsverity,
